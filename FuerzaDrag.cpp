...
const double eta = (tau - 1/2.0)/3.0;
...
public:
  ...
  void Collision(void);
  void ImposeFields(double Ufan,double R, int ixc, int iyc);
  void Advection(void);
  void Print(const char * NameFile,double Ufan);
  void Sigma(double ix, double iy, double & sigmaxx, double & sigmaxy, double & sigmayy);
  void dF_drag(double x, double y, double dAx, double dAy, double & dFx, double & dFy);
  void FuerzaTotalDrag(int NumPoints, double *Points, double *VecArea,double & Fx, double & Fy);
};
...
void LatticeBoltzmann::ImposeFields(double Ufan,double R,int ixc, int iyc){
...
void LatticeBoltzmann::Sigma(double ix, double iy, double & sigmaxx, double & sigmaxy, double & sigmayy){
  double Ux[Q]{0};
  double Uy[Q]{0};
  double rho0[Q]{0};
...
void LatticeBoltzmann::dF_drag(double x, double y, double dAx, double dAy, double & dFx, double & dFy){
  int ix = std::floor(x);
  int iy = std::floor(y);
...
void LatticeBoltzmann::FuerzaTotalDrag(int NumPoints, double *Points, double *VecArea,double & Fx, double & Fy){
  double AUXFx = 0; double AUXFy = 0;
  Fx = 0; Fy =0;
  for(int i = 0; i < NumPoints; i++){
    dF_drag(Points[2*i], Points[2*i + 1], VecArea[2*i], VecArea[2*i + 1], AUXFx, AUXFy);
    Fx += AUXFx;
    Fy += AUXFy;
  }
}
...
int main(void){
  ...
  for(t=0;t<tmax;t++){
    Aire.Collision();
    Aire.ImposeFields(Ufan0,R, ixc,iyc);
    Aire.FuerzaTotalDrag(NumPoints, Points, VecArea, FXdrag, FYdrag);
